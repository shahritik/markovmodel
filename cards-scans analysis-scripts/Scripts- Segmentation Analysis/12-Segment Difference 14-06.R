##
#Making conditional plots
#Plots are divided into section, each section representing individual segments
##

#plot histogram for gender and age
library("lattice")
histogram( ~ num_of_cat | factor(paste("Segment", c6)),data = scans1, as.table = TRUE)
histogram( ~ units | factor(paste("Segment", c6)),data = scans1, as.table = TRUE)
histogram( ~ euros | factor(paste("Segment", c6)),data = scans1, as.table = TRUE)
histogram( ~ day1 | factor(paste("Segment", c6)),data = scans1, as.table = TRUE)
histogram( ~ age | factor(paste("Segment", c6)),data = scans1, as.table = TRUE)
